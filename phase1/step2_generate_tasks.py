...
def chains_have_contact(coords_a: np.ndarray, coords_b: np.ndarray, cutoff: float) -> Tuple[bool, float]:
...
def make_directed_task(
    ...
    n_contact_residues, contact_frac, contact_ids, source_min_dist = count_source_contact_residues(
        source_ctx.residue_coords,
        source_ctx.residue_ids,
        receptor_ctx.heavy_atom_coords,
        cfg.directed_contact_cutoff,
    )
...
def build_bidirectional_tasks(cif_path: Path, cfg: Step2Config) -> List[Step2Task]:
    ...
    for i in range(len(chain_contexts)):
        for j in range(i + 1, len(chain_contexts)):
            ...
            ok, pair_min_dist = chains_have_contact(ctx_a.heavy_atom_coords, ctx_b.heavy_atom_coords, cfg.chain_contact_cutoff)
            if not ok:
                continue
            pair_id = make_pair_id(ctx_a.chain.name, ctx_b.chain.name)
            masked = [name for name in all_chain_names if name not in {ctx_a.chain.name, ctx_b.chain.name}]
...
def worker(cif_path_str: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"ok": True, "source_file": cif_path.name, "tasks": [asdict(t) for t in tasks], "error": None}
...
def read_passed_files(step1_dir: Path, pdb_dir: Path) -> List[Path]:
    ...
    for line in passed_file.read_text(encoding="utf-8").splitlines():
        ...
        if not s:
            continue
        p = Path(s)
        for candidate in (p, step1_dir / s, pdb_dir / s, pdb_dir / p.name):
            if candidate.exists():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase1 Step2: bidirectional task generation")
    ...
    (output_jsonl.parent / "step2_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
