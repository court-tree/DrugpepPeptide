...
const DECK_ID = "phase1-teacher-report";
const OUT_DIR = "E:\\pep\\phase1\\ppt_phase1_teacher";
const REF_DIR = "E:\\pep\\phase1\\ppt_phase1_teacher\\reference";
...
const SOURCES = {
  ...
  tuning: "phase1/tuning_history.md",
  length: "phase1/length_balance_experiment.md",
  len8cap: "phase1/len8cap_experiment.md",
};

const SLIDES = [
  {
    "kicker": "PEPTIDECLIP PHASE-1",
    "title": "高保真多肽-受体数据集构建流程",
    "subtitle": "从真实 PPI 复合物出发，切出结构可信、接触充分、重复度受控的多肽训练样本。",
    "expectedVisual": "Title slide with a clean scientific report visual system.",
    "moment": "核心逻辑：真实结构 → 局部片段 → 物理体检 → 联合同源去重 → 1D/3D 落库",
    "notes": "开场先强调这不是随机生成多肽，而是从真实蛋白-蛋白复合物中提取可训练样本。",
    "sources": [
      "readme"
    ]
  },
  {
    "kicker": "PIPELINE MAP",
    "title": "一条主线：先保证结构可靠，再保留代表性片段",
    "subtitle": "新版 Phase-1 去掉全局分层采样，改成更直接、更可解释的局部质量筛选链路。",
    "expectedVisual": "Seven-step editable pipeline map.",
    "cards": [
      [
        "输入端",
        "Step 1–2 先筛结构质量，再把每对接触链拆成双向 receptor / peptide-source 任务。"
      ],
      [
        "候选端",
        "Step 3–5 在真实界面附近切 8–20 aa 连续片段，用平均接触表现筛选并抽样。"
      ],
      [
        "落库端",
        "Step 6–7 用受体+多肽联合相似性去重，最后封端、提口袋并写入 1D/3D 数据。"
      ]
    ],
    "notes": "这一页适合给老师建立全局图景：流程不是单一步骤，而是一条从输入质控到最终训练数据的漏斗。",
    "sources": [
      "readme"
    ]
  },
  {
    "kicker": "CORE IDEA",
    "title": "候选生成不看单点最近，而看局部片段整体表现",
    "subtitle": "真实 PPI 界面里，单个残基最近可能只是偶然；连续片段的平均接触更接近多肽结合质量。",
    "expectedVisual": "Metric cards explaining candidate generation.",
    "metrics": [
      [
        "8–20 aa",
        "候选片段长度范围",
      ...
      [
        "avg contact",
        "以每个 peptide residue 接触到的唯一 receptor residue 均值作为核心信号",
        "减少长度偏置"
      ]
    ],
    "notes": "这里要解释为什么修正 avg_contact_count 很关键：原来量纲像原子邻居数，修正后才是残基层面的平均接触。",
    "sources": [
      "readme",
      "tuning"
    ]
  ...
  {
    "kicker": "TUNING EVIDENCE",
    "title": "调参不是盲调：先发现量纲问题，再让 Step 4 真正工作",
    "subtitle": "早期 smoke 测试暴露出 Step 4 全过的问题，关键转折点是重定义 avg_contact_count。",
    "expectedVisual": "Three-stage tuning evidence cards.",
    "cards": [
      [
        "早期问题",
        "smoke_5 到 smoke_5_v3 中，Step 4 基本 128→128 / 64→64 全部通过，说明过滤没有真正生效。"
      ],
      [
        "关键修正",
        "发现 avg_contact_count 落在 44–67，实际更像原子邻居数；改为唯一受体残基平均接触数后回到 3.56–5.50。"
      ],
      [
        "当前效果",
        "Step 4 使用 min_avg_contact_count=3.5、min_contact_coverage=0.5 后，在 pilot_100_v2 中更平衡。"
      ]
    ],
    "notes": "这页要让老师看到：不是跑通就结束，而是通过小规模 smoke 找到指标定义问题，再扩大到 pilot 验证。",
    "sources": [
      "tuning"
    ]
  },
  {
    "kicker": "CURRENT DEFAULTS",
    "title": "当前推荐默认版：简单、可解释、能控流",
    "subtitle": "参数选择优先保证物理合理性和解释性，而不是追求复杂指标堆叠。",
    "expectedVisual": "Default parameter cards.",
    "cards": [
      [
        "Step 3 / 4",
        "Step 3 每个 task 最多 16 个候选；Step 4 用 avg_contact_count ≥ 3.5 与 contact_coverage ≥ 0.5 做核心体检。"
      ],
      [
        "Step 5",
        "每个 task 最多保留 4 条；主权重仍是 avg_contact_count，并加入 max_len8_per_task=2 控制 8-mer 垄断。"
      ],
      [
        "Step 6 / 7",
        "受体 identity 与多肽 identity 均 ≥0.85 时才去重；最后按 6 Å 提取 patch，并生成最终 metadata / LMDB。"
      ]
    ],
    "notes": "这页是老师最容易追问的地方：为什么这些默认参数合理。回答核心是：小规模发现问题，大规模 pilot 验证平衡。",
    "sources": [
      "readme",
      "tuning",
      "len8cap"
  ...
  {
    "kicker": "LENGTH BIAS",
    "title": "已识别的偏置：8-mer 过多；当前用 task 内上限温和修正",
    "subtitle": "短片段更容易获得高平均接触，因此 8-mer 会天然占优；直接加长度权重有效但幅度有限。",
    "expectedVisual": "Metric cards comparing baseline and fixes.",
    "metrics": [
      [
        "77.55%",
        "baseline 最终 8-mer 占比",
        "pilot_1000"
      ],
      [
        "75.81%",
      ...
      [
        "73.32%",
        "per-task 8-mer cap 后 8-mer 占比",
        "质量基本稳定"
      ]
    ],
    "notes": "这页的重点不是说长度偏置完全解决，而是说明我们已经识别它，并选择了更容易解释的 task 内 8-mer cap。",
    "sources": [
      "length",
      "len8cap"
    ]
  },
  {
    "kicker": "TAKEAWAY",
    "title": "给老师的一句话结论",
    "subtitle": "这版 Phase-1 的目标不是一步做到分布完美，而是先构建一条稳定、可解释、可人工审查的数据生产线。",
    "expectedVisual": "Final conclusion cards.",
    "cards": [
      [
        "可靠性",
        "输入结构有质控，候选片段来自真实界面，且要求主链连续和接触充分。"
      ],
      [
        "可解释性",
        "核心指标从复杂物理量链条收敛到平均接触数、覆盖率和联合同源去重，便于检查。"
      ],
      [
        "下一步",
        "继续看全量结果的长度分布、Step 6 去重强度和代表复合物可视化，决定是否进一步调 Step 3/5。"
      ]
    ],
    "notes": "最后给老师一个稳健结论：当前版本已经适合全量稳定跑通和人工检查，后续优化有明确方向。",
    "sources": [
      "readme",
      "tuning",
      "len8cap"
...
function addReferenceCaption(slide, slideNo) {
  addText(
    slide,
    slideNo,
    "材料来源：phase1/README.md、tuning_history.md、length_balance_experiment.md、len8cap_experiment.md",
    64,
    674,
    980,
    22,
...
async function slideCover(presentation) {
  ...
  addShape(slide, "roundRect", 86, 448, 470, 126, PAPER_96, INK, 1.2, { slideNo, role: "cover moment panel" });
  addText(slide, slideNo, data.moment || "Replace with core idea", 112, 472, 416, 78, {
    size: 20,
    color: INK,
    bold: true,
    face: TITLE_FACE,
...
async function slideCards(presentation, idx) {
  ...
  for (let cardIdx = 0; cardIdx < cols; cardIdx += 1) {
    ...
    addCard(slide, idx, x, 392, cardW, 222, label, body, { iconKind: iconKinds[cardIdx % iconKinds.length] });
  }
